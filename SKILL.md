...
All council members are dispatched as pi **subagents** using the builtin `delegate` agent. Each member's persona (Identity, Grounding Protocol, Analytical Method) is embedded directly in the task prompt. No external API calls, no `.env` file, no API keys — pi's own model handles all dispatch.

- **All members inherit pi's current model** unless you specify otherwise via subagent per-task `model` overrides.
- **Model tiers**: If you want pro-tier members (Aristotle, Socrates, Aurelius, Lao Tzu, Kahneman, Taleb, Sutskever, Watts) to use a stronger model, set per-task `model` when dispatching them. Otherwise all members use pi's default model.
- **Parallel dispatch**: Use `subagent({ tasks: [...], concurrency: <N> })` for rounds that run members in parallel.
- **Sequential dispatch**: Use `subagent({ chain: [...] })` for rounds where members need prior outputs.

## Member Dispatch Pattern

All rounds use this general approach:

```typescript
subagent({
  agent: "delegate",
  task: `You are council-${memberName}. {agent Identity section}

{agent Grounding Protocol section}

{round-specific prompt content}

{relevant Output Format section}`
})
```

For parallel rounds (panel size ≥ 2):
```typescript
subagent({
  tasks: members.map(m => ({
    agent: "delegate",
    task: `You are council-${m.name}. ${m.identity}\n\n${m.roundPrompt}`
  })),
  concurrency: Math.min(members.length, 4)
})
```

For single-member fallback/retry:
- If a member's output is empty or unparseable, retry by dispatching the same subagent with the same prompt.
- If still empty, note the member as `[FAILED] {member} — response empty` and skip.
- The coordinator captures each subagent's result from the returned `output` field.
...
All council members use **pi's current model** by default. No external API configuration needed. If you want to use a specific model for the council, set it before invoking `/council` (e.g., `/model claude-sonnet-4` then `/council ...`).

**Pro-tier differentiation** (optional): If you want pro-tier members (tier `pro` in the member table) to use a stronger model, specify per-task `model` overrides in subagent dispatch calls. Otherwise, all members use pi's default model equally.
...
### STEP 2: Member Prompts

Read each member's agent file from `agents/council-{name}.md` (relative to the skill directory). Extract the **Identity**, **Grounding Protocol**, **Analytical Method**, **What You See That Others Miss**, **What You Tend to Miss**, and relevant **Output Format** sections.

Store these in the coordinator's working state — they will be embedded in each round's subagent task prompt.

`[CHECKPOINT]` State that agent files have been loaded and are ready for dispatch.
...
Dispatch each member **in parallel** using the subagent `delegate` agent:
...
**Selection:**
- The Chairman is dispatched as a subagent using `delegate` with the Chairman prompt as the task.
- The Chairman must NOT be a deliberating member in the same session (hard constraint).
- Use a different subagent dispatch or the same coordinator session for the Chairman's synthesis.
- The Chairman model used is recorded in the verdict metadata under `Chairman: <model used>`.

`[CHECKPOINT]` State the Chairman assignment and model.
...
Run all members **IN PARALLEL** using subagent `delegate` tasks. Each member sees ONLY the problem statement (blind-first, no peer outputs).

**Round 1 prompt template** (embedded in each subagent's task):
```
...
**Fallback**: If a member's output is empty or unparseable, retry once with the same prompt via a single-member subagent dispatch. If still empty, note the member as `[FAILED] {member} — response empty` and skip their contribution. Continue with remaining members.
...
**Execution strategy:**
- If panel size ≤ 4: run fully **SEQUENTIAL** using a chain (each member sees all prior Round 2 responses, still with anonymized labels)
...
**Parallel dispatch (panel ≥ 5):**
```typescript
subagent({
  tasks: members.map(m => ({
    agent: "delegate",
    task: round2Prompt(m, anonymizedOutputs)
  })),
  concurrency: Math.min(members.length, 4)
})
```

**Sequential dispatch (panel ≤ 4):**
```typescript
subagent({
  chain: members.map(m => ({
    agent: "delegate",
    task: round2Prompt(m, anonymizedOutputs, previousRound2Responses)
  }))
})
```
...
**`[VERIFY]` Dissent quota**: At least 2 members must articulate a non-overlapping objection. If fewer than 2 → send the dissent prompt via a single subagent dispatch:
```
...
**`[VERIFY]` Novelty gate**: Each response must contain at least 1 new claim, test, risk, or reframing not in that member's Round 1 output. If missing → send back via a single subagent dispatch:
```
...
**`[VERIFY]` Agreement check**: If >70% agree on core position → trigger counterfactual prompt to 2 most likely dissenters via subagent dispatch:
```
...
Dispatch all members in parallel using subagent `delegate`:
...
subagent({
  tasks: members.map(m => ({
    agent: "delegate",
    task: finalRoundPrompt(m)
  })),
  concurrency: Math.min(members.length, 4)
})
```

`[CHECKPOINT]` Collect every member's `STANCE:` line. Normalize labels that mean the same thing to a single canonical option (e.g. "monorepo" / "single repo" → `monorepo`). If a member omitted the line or it's unparseable, re-prompt that one member for the stance line only via a single subagent dispatch — do not infer their stance from prose.
...
Synthesis is performed by the **Chairman** dispatched as a subagent `delegate`. Use the full deliberation transcript as the task.
...
Capture the subagent's output as the verdict. The coordinator then surfaces the verdict to the user verbatim — no post-processing, no re-synthesis.

**Fallback**: If the Chairman subagent returns empty or unparseable output, fall back to the coordinator producing the verdict directly. Annotate the verdict metadata: `Chairman: (FAILED — synthesized by coordinator fallback)`.
...
- `model`: the pi model used for the session
...
Dispatch all members in parallel via subagent `delegate`:

```typescript
subagent({
  tasks: members.map(m => ({
    agent: "delegate",
    task: `You are operating as a council member in a rapid deliberation.
Read your agent definition at agents/council-${m.name}.md and follow it precisely.
...
subagent({
  tasks: members.map(m => ({
    agent: "delegate",
    task: quickRound2Prompt(m, anonymizedOutputs)
  })),
  concurrency: Math.min(members.length, 4)
})
```
...
Dispatch synthesis to the Chairman using the subagent `delegate` method (same as full mode STEP 9). Use the Quick Verdict template below. Same fallback rule as STEP 9.
...
Dispatch both members in parallel via subagent `delegate`:

```typescript
subagent({
  tasks: [memberA, memberB].map(m => ({
    agent: "delegate",
    task: `You are operating as one half of a structured dialectic with one opponent.
Read your agent definition at agents/council-${m.name}.md and follow it precisely.
...
Dispatch synthesis to the Chairman via the subagent `delegate` method. In duo mode the Chairman must NOT be either of the two duo members (hard constraint — Chairman audits, not participates). Use the Duo Verdict template below. Same fallback rule as STEP 9.
